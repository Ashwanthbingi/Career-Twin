...
const LEETCODE_API_BASE_URL = import.meta.env.VITE_API_URL || "http://localhost:8080";
...
export async function analyzeLeetCodeProfile(request: LeetCodeAnalysisRequest): Promise<LeetCodeIntelligenceResponse> {
  const url = `${LEETCODE_API_BASE_URL}/api/leetcode/analyze`;
  const response = await fetch(url, {
    method: "POST",
...
export async function fetchLeetCodeProfile(userId: number): Promise<LeetCodeIntelligenceResponse | null> {
  const url = `${LEETCODE_API_BASE_URL}/api/leetcode/profile?userId=${userId}`;
  const response = await fetch(url);
  if (response.status === 404) return null;
  if (!response.ok) {
    const errorText = await response.text();
